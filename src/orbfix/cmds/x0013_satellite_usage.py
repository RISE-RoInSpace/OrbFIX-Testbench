...
app = typer.Typer(
    help="Get or set which satellites are allowed to be included in the PVT computation."
)
...
def _parse_satellite_usage(decoded):
    """
    Command 0x0013: Satellite usage (Get/Set)
    Payload:
    ...
    result = f"Satellite usage: (bitfield=0x{bitfield:08X})"
...
def set_satellite_usage(
    ...
    Examples:
        ...
        orbfix cmd satellite-usage set --default_sats
        orbfix cmd satellite-usage set --ds

        # Enable a certain constelation
        orbfix cmd satellite-usage set --constelation Galileo
        orbfix cmd satellite-usage set -cstl GPS -cstl Galileo

        # Enable a certain satellite
        orbfix cmd satellite-usage set --satellite G01
        orbfix cmd satellite-usage set -stl G02 -stl J02
    ...
    if payload:
        ...
        typer.secho("\nSatellite Usage Configuration:", fg="cyan", bold=True)
